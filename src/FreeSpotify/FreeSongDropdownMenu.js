...
function FreeSongDropdownMenu({accessToken, track, showPlaylistDropDown, setSelectedArtistId, setSelectedPanel, setSelectedAlbum, playlists, setSearch, debouncedSearchInput, setPlaylists, user}) {
        useEffect(() => {
    ...
    return (
        ...
        <div id={`playlistDropdown-${track.id}`} className="dropdown-content">
                ...
                <>
                        <a onClick={()=>handleAddToPlaylist(plist.id, track.uri, track.id)}>{plist.name}</a>
                </>
                ))}
        </div>
        </>
...
export default FreeSongDropdownMenu;